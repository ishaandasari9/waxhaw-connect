...
const copy = {
  en: {
    ...
    location: 'Charlotte, Waxhaw & nearby communities', tagline: 'A stronger community, together.',
    hero: 'Find support. Join in. Feel at home.',
    heroSub: 'Compare trusted resources, events and opportunities around where you live.',
    homeLocation: { eyebrow: 'Local to you', title: 'See what is closest to you', intro: 'Enter a ZIP code from Charlotte, Waxhaw or a nearby community, or use your device location.' },
    ...
    upcoming: 'Upcoming nearby', viewAll: 'View all resources', openNow: 'Open now', closedNow: 'Closed now',
  ...
  es: {
    ...
    location: 'Charlotte, Waxhaw y comunidades cercanas', tagline: 'Una comunidad más fuerte, unida.',
    hero: 'Encuentre apoyo. Participe. Siéntase en casa.',
    heroSub: 'Compare recursos, eventos y oportunidades confiables cerca de donde vive.',
    homeLocation: { eyebrow: 'Cerca de usted', title: 'Vea qué queda más cerca', intro: 'Ingrese un código postal de Charlotte, Waxhaw o una comunidad cercana, o use la ubicación de su dispositivo.' },
    ...
    upcoming: 'Próximamente cerca', viewAll: 'Ver todos los recursos', openNow: 'Abierto ahora', closedNow: 'Cerrado ahora',
    ...
    <>
      <section className="hero">
        ...
        <div className="hero__content">
          <span className="eyebrow"><MapPin size={15} /> {t.location}</span>
      ...
      <section className="section home-location distance-bar" aria-labelledby="home-location-title">
        <div className="home-location__intro">
          <span className="eyebrow">{t.homeLocation.eyebrow}</span>
          <h2 id="home-location-title">{t.homeLocation.title}</h2>
          <p>{t.homeLocation.intro}</p>
        </div>
        <LocationPicker compact labels={t.distance} />
      </section>
      <UrgentBand />
...
function FeaturedResource() {
  ...
  return (
    <article className="featured-resource">
      ...
      <div className="resource-meta"><span><MapPin /> {resource.location}</span><DistanceTag item={resource} labels={t.distance} /><span><Clock3 /> {resource.hours}</span></div>
...
function EventRow({ event }) {
  ...
  return <div className="event-row"><time dateTime={event.date}><strong>{date.toLocaleDateString(language, { month: 'short' })}</strong><span>{date.getDate()}</span></time><div><strong>{event.title}</strong><small>{event.time} · {event.location}</small><DistanceTag item={event} labels={t.distance} /></div></div>
}
...
function FinderPage() {
  ...
  const results = useMemo(() => {
    ...
    return matching.sort((a, b) => {
      ...
      if (answers.priority === 'local') return 0
