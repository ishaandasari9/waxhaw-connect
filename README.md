...
- Recurring listings: a town meeting is stored as its published schedule ("second Tuesday") rather than a fixed date, so the calendar always shows the next meeting of each board and never empties out as dates pass
...
`python3 scripts/build-color-fallbacks.py` rewrites the `@supports` block at the end of `src/styles.css` after any color token changes. Icons are generated from `public/icon.svg`.
